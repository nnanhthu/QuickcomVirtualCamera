//
//  CMIO_DP_QuickomCamera_PlugInInterface.cpp
//  Sample Plugin
//
...
{
	//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// AppleCMIODPQuickomCameraPlugIn()
	//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	void* AppleCMIODPQuickomCameraPlugIn(CFAllocatorRef allocator, CFUUIDRef requestedTypeUUID);
	void* AppleCMIODPQuickomCameraPlugIn(CFAllocatorRef allocator, CFUUIDRef requestedTypeUUID)
	{
		...
		try
		{
			...
			name_t assistantServiceName = "com.apple.cmio.DPA.QuickomCamera";
			...
			if (BOOTSTRAP_SUCCESS != err)
			{
				...
				CACFURL assistantURL(CFURLCreateWithFileSystemPath(NULL, CFSTR("/Library/CoreMediaIO/Plug-Ins/DAL/QuickomCamera.plugin/Contents/Resources/QuickomCameraAssistant"), kCFURLPOSIXPathStyle, false));
				ThrowIf(not assistantURL.IsValid(), CAException(-1), "AppleCMIODPQuickomCameraPlugIn: unable to create URL for the QuickomCameraAssistant");
				...
				ThrowIf(BOOTSTRAP_SUCCESS != err, CAException(err), "AppleCMIODPQuickomCameraPlugIn: couldn't create server");
				...
				ThrowIf(BOOTSTRAP_SUCCESS != err, CAException(err), "AppleCMIODPQuickomCameraPlugIn: couldn't create QuickomCameraAssistant service port");
			}
