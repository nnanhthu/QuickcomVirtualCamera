...
namespace
{
	...
	boolean_t MessagesAndNotifications(mach_msg_header_t* request, mach_msg_header_t* reply)
	{
		...
		{
			Server::VCamAssistant::Instance(0)->ClientDied(request->msgh_local_port);
			processed = true;
		}
...
//        std::cout << argv[1] << std::endl;
		...
		name_t serviceName = "com.apple.cmio.DPA.QuickomCamera";
		...
		mach_port_t portSet = VCamAssistant::Instance(type)->GetPortSet();
