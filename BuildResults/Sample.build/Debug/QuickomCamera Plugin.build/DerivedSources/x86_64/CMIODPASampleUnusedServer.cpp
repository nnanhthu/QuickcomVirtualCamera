/*
 * IDENTIFICATION:
 * stub generated Thu Jun 24 21:57:28 2021
...
#include "CMIODPASampleUnusedServer.h"
...
#pragma pack(pop)
	...
	RetCode = CMIODPASampleGetDeviceStates(In0P->Head.msgh_request_port, In0P->messagePort.name, (CMIO_SCOPE_DPA_SCOPE_Sample_SCOPE_DeviceStatePtr *)&(OutP->states.address), &OutP->statesCnt);
	if (RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, RetCode);
	}
...
#pragma pack(pop)
	...
	RetCode = CMIODPASampleGetProperties(In0P->Head.msgh_request_port, In0P->guid, In0P->messagePort.name, In0P->time, In0P->matchAddress, (CMIO_SCOPE_PropertyAddressPtr *)&(OutP->addresses.address), &OutP->addressesCnt);
	if (RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, RetCode);
	}
...
#pragma pack(pop)
	...
	RetCode = CMIODPASampleGetControls(In0P->Head.msgh_request_port, In0P->guid, In0P->messagePort.name, In0P->time, (CMIO_SCOPE_DPA_SCOPE_Sample_SCOPE_ControlChangesPtr *)&(OutP->controlChanges.address), &OutP->controlChangesCnt);
	if (RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, RetCode);
	}
